class Data_Loader:
    ...
    def load_alpha(self,sub=-1,fmin=0.5,fmax=50,sfreq=100):
    ...
    def load_faller(self,sub = -1,fmin=8,fmax=13,fsamp = 100):
        import moabb
        import numpy as np
...
class Experiment:
    ...
    def subject_independent_cv(X,y,baselines =['covariance','raw','fft2','fft','cross_correlate','maxmin','meanstd','variance','spectral_entropy','all']
                               ...
                               ,verbose = 0,inner_splits=1,seed = 0,n_gens = 10,k=5,n_pops=18,n_save = 5,model_name='model_name'):
        import numpy as np
        from sklearn.model_selection import StratifiedKFold,train_test_split
        import EEGene
        import Benchmarks
        from EEGene import Processing,EEGene
        from Benchmarks import EEGNet,MIN2Net,DeepConvNet
        import shutil
        import os
        def gen_indep_subjects(n_subjects,seed=0,k=5):
            import random
            random.seed(seed)
            ans = []
            for test in range(n_subjects):
                temp = []
                ans.append([])
                for j in range(min(k,n_subjects-1)):
                    val = random.choice(range(n_subjects))
                    while val == test or val in temp:
                        val = random.choice(range(n_subjects))
                    temp.append(val)
                    ans[-1].append({'test':[test],'val':[val],'train':[i for i in range(n_subjects) if i!=test and i!=val]})
            return ans
        def make_df (scores):
            import numpy as np
        ...
        subjects = len(X)
        gene_reports = []
        indep_subjects_for_NN = gen_indep_subjects(n_subjects = len(X),k=inner_splits)
        for test_sub in range(subjects):
            ...
            for bench in benchmarks:
                ...
                for inner_k in indep_subjects_for_NN[test_sub]:
                    test_sub = inner_k['test'][0]
                    val_sub = inner_k['val'][0]
                    train_sub = inner_k['train']
                    X_test = X[test_sub]
                    y_test = y[test_sub]
                    X_val = X[val_sub]
                    y_val = y[val_sub]
                    ...
                    for sub in range(subjects):
                        if sub in train_sub:
                            if type(X_train) == type(None):
                                ...
                                y_train = np.concatenate((y_train,y[sub]))
                    X_test = P.scaler(X_test)
                    X_val = P.scaler(X_val)
                    X_train = P.scaler(X_train)
                    trials,chs,samples = X_train.shape
                    if bench == 'EEGNet':
                        try:
                            print('deleting')
                            shutil.rmtree('log/EEGNet'+model_name+'_out_weights.tf')
                            os.remove('log/EEGNet'+model_name+'_out_log.log')
                            os.remove('log/EEGNet'+model_name+'_time_log.csv')
                        except:
                            print('no delete')
                        finally:
                            X_EEGNet_train = np.expand_dims(X_train,-1)
                            X_EEGNet_val = np.expand_dims(X_val,-1)
                            ...
                            model.fit(X_EEGNet_train, y_train, X_EEGNet_val, y_val)
                    ...
                    if bench == 'DeepConvNet':
                        try:
                            print('deleting')
                            shutil.rmtree('log/DeepConvNet'+model_name+'_out_weights.tf')
                            os.remove('log/DeepConvNet'+model_name+'_out_log.log')
                            os.remove('log/DeepConvNet'+model_name+'_time_log.csv')
                        except:
                            print('no delete')
                        finally:
                            X_DeepConvNet_train = np.expand_dims(X_train,-1)
                            X_DeepConvNet_val = np.expand_dims(X_val,-1)
                            ...
                            model.fit(X_DeepConvNet_train, y_train, X_DeepConvNet_val, y_val)
                    ...
                    if bench == 'MIN2Net':
                        try:
                            print('deleting')
                            shutil.rmtree('log/MIN2Net'+model_name+'_out_weights.tf')
                            os.remove('log/MIN2Net'+model_name+'_out_log.log')
                            os.remove('log/MIN2Net'+model_name+'_time_log.csv')
                        except:
                            print('no delete')
                        finally:
                            X_MIN2Net_train = np.expand_dims(X_train,-1)
                            X_MIN2Net_train = np.transpose(X_MIN2Net_train,(0,3,2,1))
                            X_MIN2Net_val = np.expand_dims(X_val,-1)
                            ...
                            model.fit(X_MIN2Net_train, y_train, X_MIN2Net_val, y_val)
            ...
            np.save('final_report/'+model_name+'.npy',scores)
        return scores
    ...
    def single_subject_dependent_cv(X,y,baselines=['covariance','raw','fft2','fft','cross_correlate','maxmin','meanstd','variance','spectral_entropy','all'],
                             ...
                             inner_splits=1,seed = 0,n_gens = 10,k=5,n_pops=18,n_save = 5,model_name='model_name'):
        import numpy as np
        from sklearn.model_selection import StratifiedKFold,train_test_split
        import EEGene
        import Benchmarks
        ...
        for train_index, test_index in skf.split(X, y):
            ...
            for bench in benchmarks:
                ...
                if inner_splits == 1:
                    ...
                    X_inner_train, X_inner_val, y_inner_train, y_inner_val = train_test_split(X_train, y_train, test_size=0.25, random_state=seed)
                    ...
                    inner_skf = StratifiedKFold(n_splits=inner_splits, random_state=seed, shuffle=True)
        ...
        np.save('final_report/'+model_name+'_single_subject.npy',scores)
        return scores
...
#                              benchmarks=['EEGNet','DeepConvNet','MIN2Net','EEGene'],verbose = 0,
#                              inner_splits=5,seed = 0,n_gens = 10,k=5,n_pops=18,n_save = 5,model_name='model_name',
#                              MIN2Net_lr=0.01,MIN2Net_min_lr=0.001,MIN2Net_patience=5
#                                    ):
#         import numpy as np
#         from sklearn.model_selection import StratifiedKFold,train_test_split
#         import EEGene
...
#             X = Xall[sub]
#             y = yall[sub]
#             skf = StratifiedKFold(n_splits=inner_splits, random_state=seed, shuffle=True)
#             skf.get_n_splits(X, y)
#             P = Processing()
#             batch = 0
#             for train_index, test_index in skf.split(X, y):
...
#                                 X_MIN2Net_test = np.expand_dims(X_test_scaled,-1)
#                                 X_MIN2Net_test = np.transpose(X_MIN2Net_test,(0,3,2,1))
#                                 model = MIN2Net(input_shape=(1, samples, chs), num_class=2, monitor='val_loss', shuffle=True,batch_size=trials//4,verbose=0,model_name=bench+model_name,lr=MIN2Net_lr,min_lr=MIN2Net_min_lr)
#                                 model.fit(X_MIN2Net_train, y_inner_train, X_MIN2Net_val, y_inner_val)
#                                 _, evaluation3 = model.predict(X_MIN2Net_test, y_test)
#                                 scores['MIN2Net'].append(evaluation3)
#                                 shutil.rmtree('log/MIN2Net'+model_name+'_out_weights.tf')
...
#         np.save('final_report/'+model_name+'_all_subjects.npy',scores)
#         return scores
